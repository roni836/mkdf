...
@section('content')
    <div class="  flex justify-center items-center h-screen">
        <div class="bg-gray-100 p-8 rounded-lg shadow-md w-full max-w-lg">
            <h2 class="text-2xl font-bold mb-6 text-center">Add Story Form</h2>
            <form action="#" method="POST" class="space-y-4 " id="addData">
                ...
                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700">Story Image</label>
                ...
                <div>
                    ...
                    <textarea id="story" name="story" rows="2"
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                    <p id="error-story" class="text-red-500 text-xs font-semibold error-message"></p>
                </div>
                <div>
                    <button type="submit" class="w-full bg-indigo-600 text-white py-2 px-4 rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Add Story Now</button>
                </div>
            </form>
        </div>
    </div>
            ...
            $("#addData").submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        window.open("/admin/manage-story", "_self");
                    },
                    error: function(xhr) {
                        $('.error-message').html('');
                        if (xhr.status === 422) {
